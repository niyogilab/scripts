...
msr <- read.csv("/home/daniel/Sync/Google Drive/Niyogi Lab/Zofingiensis/Switch Paper/Switch Paper Part B/Replot/oxygen_evolution.csv", header=TRUE,as.is=TRUE)
msr$strain = factor(msr$strain, levels= c("wt", "hxk1-1", "hxk1-2"))
msr1 <- msr %>% group_by(strain,glc) %>%  dplyr::summarise(average = mean(O2evol), sd = sd(O2evol))
...
p2<- ggplot(msr1, aes(x=strain,y=average,fill=glc,ymin=average-sd,ymax=average+sd))+
  ...
  geom_point(data=msr,aes(x= strain,y=O2evol,group=msr$interact),inherit.aes = FALSE,shape=21,color="black",fill="white",position = position_dodge(width=0.5),alpha = 0.7,size=4,stroke=1.1)+
...
p2+scale_x_discrete(labels = xlabels2)+ggtitle("Net Oxygen Evolution")+ ylab(label = paste("Net O2 Evolution")) + 
  xlab(label = " ")+ theme_classic(base_size = 20)+
  theme(plot.title = element_text(color = "black", size = 30, hjust = 0.5)) +
  scale_y_continuous(breaks = seq(-8,16,4),limits = c(-8,16),expand=c(0,0)) +
